...
def create_matplotlib_map(df, point_size, title):
    ...
    sc = m.pcolormesh(x, y, ch_grid, cmap='rainbow')
...
if st.session_state.data is not None and len(st.session_state.combinations) > 0:
    ...
    if df_display.empty:
        ...
        if map_library_choice == "Plotly":
            ...
            fig = create_matplotlib_map(df_display, point_size, f"DK Rainfall_{selected_combo}")
